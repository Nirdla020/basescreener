  ...
  await Promise.all(
    addresses.map(async (addr) => {
      try {
        ...
        const p = pairs.find((x: any) => String((x as any)?.chainId || "") === "base") || pairs[0];
        if (!p) return;
...
export default function FeaturedHighlights({ tokens }: { tokens: Token[] }) {
  const [featuredItems, setFeaturedItems] = useState<FeaturedItem[]>([]);
  const [featuredInfo, setFeaturedInfo] = useState<Record<string, Partial<Token>>>({});
    ...
    return featuredItems
      ...
      .map((x) => {
        ...
        const merged: Token = {
          ...
          change: typeof fromDashboard?.change === "number" ? fromDashboard.change : (fromDex?.change as any),
          volume: typeof fromDashboard?.volume === "number" ? fromDashboard.volume : (fromDex?.volume as any),
        };
  ...
  function FeaturedRow(t: Token & { _featured?: FeaturedItem }, i: number) {
    ...
    const promotedWeights = featuredItems.filter((x) => x.promoted).map((x) => x.weight);
    const maxPromotedWeight = promotedWeights.length ? Math.max(...promotedWeights) : 0;

    const isTopSponsor = meta?.promoted && (meta?.weight || 0) === maxPromotedWeight;

    const label =
      t.symbol && t.name ? `${t.symbol} — ${t.name}` : t.symbol || t.name || meta?.title || "Featured";
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="truncate font-semibold flex items-center gap-2">
                <span className="truncate">{isTopSponsor ? "👑" : "🔥"} {label}</span>
              </div>
    ...
    return (
      <Link
        key={addr + i}
        href={`/token/${addr}`}
        onClick={() => trackClick(meta?.promoted ? "featured_promoted" : "featured", addr)}
  ...
  if (featuredTokens.length === 0) return null;
