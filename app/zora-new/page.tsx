...
export const metadata = {
  title: "Zora New Launches",
  description: "Newest token launches on Zora (Base preferred).",
  alternates: { canonical: "https://basescreener.fun/zora-new" },
};
...
function identiconDataUri(seed: string) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < Math.ceil(size / 2); x++) {
      ...
      rects += `<rect x="${x1}" y="${y1}" width="${cell}" height="${cell}" rx="6" fill="${
        x % 2 === 0 ? c1 : c2
      }" />`;

      if (x2 !== x1) {
        rects += `<rect x="${x2}" y="${y1}" width="${cell}" height="${cell}" rx="6" fill="${
          x % 2 === 0 ? c1 : c2
        }" />`;
      }
    }
...
export default async function ZoraNewPage() {
  let coins: any[] = [];
  let error = "";
